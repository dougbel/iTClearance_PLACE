...
def execute_place_in_picked_point(data_dir, dataset_name, scene_name, np_point, visualize = True):
    ...
    for step in tqdm(range(itr_s2)):
        ...
        if (s_grid_min_batch is None):
            ...
            print("warning: no considering collision loss")
        else:
...
if __name__ == '__main__':
    ...
    if not counter_follow_up_column in follow_up_data.columns:
        follow_up_data[counter_follow_up_column] = 0
    if not goal_follow_up_column in follow_up_data.columns:
        follow_up_data[goal_follow_up_column] = 10

    num_total_task = follow_up_data.index.size
    pending_tasks = list(follow_up_data[(follow_up_data[counter_follow_up_column] < follow_up_data[goal_follow_up_column])].index)
    ...
    for dataset_name, scene_name in pending_tasks:
        ...
        transform = np.eye(4)
        transform[:3, 3] = trimesh_env.bounds.mean(axis=0)
        bb_padded = trimesh.primitives.Box(transform=transform, extents=trimesh_env.extents * 1.5, mutable=False)
        scene_max_x, scene_max_y = bb_padded.vertices[:, :2].max(axis=0)
        scene_min_x, scene_min_y = bb_padded.vertices[:, :2].min(axis=0)

        trimesh_env.visual.face_colors = trimesh_env.visual.face_colors
        for i in range(num_place_picked_sampled, goal_place_picked_sampled):
            selected_p = None
            ...
            while inter_type not in ["1","2","3","4","5"]:
                ...
                print("1) laying")
                print("2) reaching out")
                print("3) sitting")
                print("4) standing up")
                print("5) walking")
                inter_type = input("What kind of interaction was found by PLACE: ")
            ...
            if inter_type=="1":
                affected_column = "num_samples_laying"
            elif inter_type=="2":
                affected_column = "num_samples_reaching_out"
            elif inter_type == "3":
                affected_column = "num_samples_sitting"
            elif inter_type == "4":
                affected_column = "num_samples_standing_up"
            elif inter_type == "5":
                affected_column = "num_samples_walking"

            num_sampled_type = follow_up_data.at[(dataset_name, scene_name), affected_column]
            num_sampled_type += 1
            follow_up_data.at[(dataset_name, scene_name), affected_column] = num_sampled_type

            num_place_picked_sampled += 1
            follow_up_data.at[(dataset_name, scene_name), counter_follow_up_column] = num_place_picked_sampled
