...
if __name__ == '__main__':
    ...
    # python examples/comparisson/testing_PLACE_then_iTC/20_gif_generator.py --base_dir /media/apacheco/Ehecatl/PLACE_comparisson --dataset prox --env_name MPH16 --num_point 0
    ...
    samples_it_opti_down_dir = opj(base_dir, "test_place_picker", "sampled_it_clearance_opti_down_trans")
    ...
    if (dataset_name, env_name, num_point) in completed_task:
        ...
        it_opti_down_subdir = opj(samples_it_opti_down_dir, env_name)
        ...
        for current_interaction in interactions_by_type[interaction_type]:
            ...
            if os.path.exists(opj(samples_it_dir, env_name, file_name)):
                ...
                gc.collect()
                trimesh_body = trimesh.load(opj(it_opti_down_subdir, f"body_{num_point}_{current_interaction}.ply"))
                generate_gif(trimesh_env, trimesh_body, view_center, opj(output_subdir, f"body_{num_point}_{current_interaction}_opti_down.gif"))
                gc.collect()
