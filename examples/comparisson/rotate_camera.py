...
def generate_gif(trimesh_env, trimesh_body,view_center,  save_on_file=None):
    ...
    v = Viewer(scene,
               ...
               viewer_flags={"rotate": True, "rotate_axis": [0, 0, 1], "view_center": view_center,
...
if __name__ == '__main__':
    ...
    for dataset_name, env_name, interaction in pending_tasks:
        ...
        for np_point_file_name in get_files_with_extension(it_subdir, ".npy"):
            ...
            place_b0_orig = trimesh.load(opj(place_subdir, f"body_{n}_orig.ply"))
            generate_gif(trimesh_env, place_b0_orig, np_point, opj(output_subdir,f"body_{n}_orig.gif"))
            place_b0_orig = trimesh.load(opj(place_subdir, f"body_{n}_opt1.ply"))
            generate_gif(trimesh_env, place_b0_orig, np_point, opj(output_subdir,f"body_{n}_opt1.gif"))
            place_b0_orig = trimesh.load(opj(place_subdir, f"body_{n}_opt2.ply"))
            generate_gif(trimesh_env, place_b0_orig, np_point, opj(output_subdir,f"body_{n}_opt2.gif"))

            place_b0_orig = trimesh.load(opj(it_subdir, f"body_{n}.ply"))
            generate_gif(trimesh_env, place_b0_orig, np_point, opj(output_subdir,f"body_{n}.gif"))
