...
PAR_NUM_SAMPLES_TO_GENERATE = 10
PAR_SAVING_OUPUTS = True
PAR_VISUALIZING_OUTPUT = True
...
for i in range(PAR_NUM_SAMPLES_TO_GENERATE):
    ...
    if PAR_SAVING_OUPUTS:
        ...
        body_trimesh_sampled.export(opj(output_dir, "body_sample.ply"))
        np.save(opj(output_dir, "cage_pc_selected_local"), selected_scene_verts_local)
        data = {
            "scene": scene_name,
            "rotate_angle1": rot_angle_1,
            "shift": list(shift),  # - selected_p
            "cube_rotated": PAR_ROTATE_CUBE,
            "eps": list(eps.detach().cpu().numpy().squeeze().astype('float'))
        }
        with open(opj(output_dir, 'data.json'), 'w') as f:
            json.dump(data, f)

    if PAR_VISUALIZING_OUTPUT:
